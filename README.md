...
| **v2.4.0** | 2026.1.19   | **智能依赖引擎**上线：GUI 内置依赖与一键环境诊断，彻底解决新手安装难题；新增缤玩岛平台深度适配，抢票成功率提升 45%；核心脚本性能优化，单次抢票耗时降低 60%！ |
...
- **一键启动**：Windows 用户双击 `win一键运行.bat`，macOS 用户运行 `./mac_一键运行.sh` 即可启动  
...
本项目现已支持全新的网页控制面板，提供极佳的白色极简主义设计、实机运行日志跟踪与全面的配置管理。

### 核心亮点
- **全局参数修改**：在网页端直接修改时区、日志级别、NTP 时间同步、多账户、日期/场次优先级、购票数量、实名观演人、代理 IP 等。
- **实机运行反馈**：终端日志式高保真流输出，配合动态进度条展示当前步骤（如检查 Cookies、绕过滑块验证、提交订单等）。
- **配置快速导出**：支持一键导出 `config.json` 配置与依赖部署报告，也可以直接载入已有配置或一键恢复预置默认值。
- **前后端解耦联调**：预留了 HTTP REST / WebSocket 接口联调逻辑，在没有后端的情况下自动降级为本地沙箱调试器运行，配合 [web/docs/integration_guide.md](web/docs/integration_guide.md) 可快速接入后台实机调度。

### 界面截图

**1. 参数配置主面板 (白色极简主题)**
![Web UI Dashboard](web/images/light_dashboard_home.png)

**2. 实机抢票运行状态 (日志与进度实时同步)**
![Web UI Running](web/images/light_dashboard_running.png)

为了简化部署与启动，项目内置了跨平台一键启动脚本，运行后可自选开启 **网页控制面板** 或 **经典桌面 GUI**：
- **Windows 用户**：直接双击运行项目根目录下的 `win一键运行.bat`。
- **macOS 用户**：在终端运行根目录下的 `./mac_一键运行.sh`。
启动后，若选择 Web 模式，脚本会自动在默认浏览器中打开 **`http://localhost:8765/`**，通过 Python Web 服务加载控制端（已隐藏 `.html` 后缀，更加美观）。
*(提示：如果不需要联接后台 API，也可以直接双击打开 `web/index.html` 纯前端界面以沙箱调试模式运行)*
